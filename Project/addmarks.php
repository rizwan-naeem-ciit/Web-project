...
<?php
  if(isset($_GET['addm-submit'])){
    require 'header.php';
    require 'includes/connection.inc.php';

    $regno = $_GET['regno'];
    if (empty($regno)) {
      header("location: coordinator.php?addmarks-submit=&regno=".$regno."&error=emptyfields&courseid=".$courseid."&lectureno=".$lectureno."&date=".$date."&status".$status);
      exit();
    }
    elseif (!preg_match("/^[0-9]*$/", $regno)) {
      header("location: coordinator.php?addmarks-submit=&regno=".$regno."&error=invalidregno&courseid=".$courseid."&lectureno=".$lectureno."&date=".$date."&status".$status);
      exit();
    }
    else {
      ...
      $stmt = mysqli_stmt_init($conn);
        if(!mysqli_stmt_prepare($stmt, $sql)){
          header("Location: coordinator.php?addmarks-submit=&regno=".$regno."&error=sqlerror");
          exit();
        }
        else {
          ...
          if($resultCheck === 0){
            header("location: coordinator.php?addmarks-submit=&regno=".$regno."&error=studentnotadd&courseid=".$courseid."&lectureno=".$lectureno."&date=".$date."&status".$status);
            exit();
          }
          else {
            echo '<div align="right"><nav>
              ...
              <li><a href="coordinator.php?addmarks-submit=">Back</a></li>
              </ul>
              ...
              if($resultCheckreg === 0){
                ...
                While(mysqli_stmt_fetch($stmtreg)){
                  ...
                  if(isset($_GET['error'])){
                    ...
                    elseif($_GET['error']== "invaliddeliverable"){
                      echo"<p>Invalid Deliverable name</p>";
                    }
                    elseif($_GET['error']== "alreadyadded"){
                      echo"<p>Record is edited</p>";
                    }
                  }
                  else if(isset($_GET['addmarks'])){
                    if ($_GET['addmarks'] == "success") {
                      echo"<p>Marks is added</p>";
                    }
                  }
                  echo '<div class="sigup"><br><br><form  action="includes/addmarks.inc.php" method="post">
                  ...
                  <input type="text" name="deliverable" placeholder="Deliverable name(in capital)"><br>
                  <input type="text" name="total-marks" placeholder="Total marks"><br>
                  <input type="text" name="obtained-marks" placeholder="Obtained marks"><br>
                  <button type="submit" name="addMarks-submit" class="btn btn-primary btn-block btn-large">Enter</button>
                </form></div>';
              }
            }
          }
